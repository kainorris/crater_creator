...
typedef struct {
	...
	uint16_t tp_dss_max_total_rate_per_spad_mcps;
	uint16_t tp_dss_rec_total_rate_per_spad_mcps;
...
typedef struct {
	...
	uint32_t	current_samples[VL53L1_MAX_USER_ZONES];


	uint32_t	required_samples;


	uint64_t	accumulator[VL53L1_MAX_USER_ZONES];


	uint32_t	nodetect_counter[VL53L1_MAX_USER_ZONES];

	uint32_t	previous_xtalk[VL53L1_MAX_USER_ZONES];
...
typedef struct {
	...
	int32_t multi_bins_rec[VL53L1_MAX_USER_ZONES][VL53L1_BIN_REC_SIZE]
		[VL53L1_TIMING_CONF_A_B_SIZE][VL53L1_HISTOGRAM_BUFFER_SIZE];

	int16_t PreviousRangeMilliMeter[VL53L1_MAX_USER_ZONES];
	uint8_t PreviousRangeStatus[VL53L1_MAX_USER_ZONES];
	uint8_t PreviousExtendedRange[VL53L1_MAX_USER_ZONES];
	uint8_t PreviousRangeActiveResults[VL53L1_MAX_USER_ZONES];
