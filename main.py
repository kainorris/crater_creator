...
class CraterCreator:
    def __init__(self):
        ...
        self.map_img = pygame.transform.scale(self.map_img, (MAP_W - (MAP_MARGIN * 2), MAP_H - (MAP_MARGIN * 2)))
    ...
    def draw_canvas(self):
        ...
        stats = [("SENSOR m/s", f"{self.sensor_vel:.2f}", CYAN), ("ASTEROID km/s", f"{self.asteroid_vel:.2f}", GOLD), ("CRATER MILES", f"{self.crater_miles:.2f}", WHITE)]
        ...
        for i, (l, v, c) in enumerate(stats):
            ...
            self.canvas.blit(self.font_data_val.render(v, True, WHITE), (50 + i*750, 165))
    ...
    def run(self):
        while True:
            for event in pygame.event.get():
                ...
                # Generate random velocity (CHANGE WHEN VEL SENSOR IS IMPLEMENTED)
                if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE:
                    self.sensor_vel = round(random.uniform(2.8, 14.8), 2)
                    ...
                    self.impact_pos = (
                        ...
                        random.randint(TOP_BAR_H + MAP_MARGIN + 200, VIRTUAL_H - MAP_MARGIN - 200)
                    )
                    
                    # Add to leaderboard if necessary and set flash to occur
                    self.leaderboard.append({'vel': self.crater_miles})
                    self.leaderboard = sorted(self.leaderboard, key=lambda x: x['vel'], reverse=True)[:10]
            ...
            if self.flash_alpha > 0:
                ...
                f.fill(WHITE); f.set_alpha(self.flash_alpha)
